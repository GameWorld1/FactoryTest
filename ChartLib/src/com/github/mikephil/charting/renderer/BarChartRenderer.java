    ...
    protected void drawDataSet(Canvas c, BarDataSet dataSet, int index) {
        ...
        if (dataSet.getColors().size() > 1) {
            ...
            for (int j = 0; j < buffer.size(); j += 4) {
                ...
                int entryIndex = j / 4;
                BarEntry entry = entries.get(entryIndex);
...
//                mRenderPaint.setColor(dataSet.getColor(j / 4));
                if (null != mFixSet) {
                    String xValue = mChart.getBarData().getXVals().get(j / 4);
                    if (null != xValue && !"".equals(xValue) && !mFixSet.contains(Integer.valueOf(xValue))) {
            ...
            for (int i = 0; i < mChart.getBarData().getDataSetCount(); i++) {
                ...
                if (!dataSet.isStacked()) {
                    ...
                    for (int j = 0; j < valuePoints.length * mAnimator.getPhaseX(); j += 2) {
                        ...
                        float y = valuePoints[j + 1] + (val >= 0 ? posOffset : negOffset);
                        String xValue = mChart.getBarData().getXVals().get(j / 2);
                        drawValue(c, dataSet.getValueFormatter(), val, entry, i, x, y);
                        drawIcon(c, xValue, x, y);
